...
function getTrain() {
    ...
    var origin = 'Entrecampos';
    var destination = "Benfica";
    var date = '2015-05-30';
    $.ajax({
            method: 'POST',
            url: url,
        ...
        .success(function(data) {
            renderStatus('Pedido efectuado com sucesso! Aguarde...');
            var trains = parseData(data);
...
function findNextTrain(trains) {
    ...
    $.each(trains, function(index, train) {
        console.log(train.departureTime.isAfter(currentTime));
...
function handleNextTrain(train) {
    ...
    renderStatus('O próximo comboio parte de Entrecampos às ' + departureTime + ' e chega a Benfica às ' +
